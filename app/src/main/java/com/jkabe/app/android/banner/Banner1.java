...
import com.jkabe.app.android.banner.listener.OnBannerClickListener;
import com.jkabe.app.android.banner.listener.OnBannerListener;
import com.jkabe.app.android.banner.loader.ImageLoaderInterface;
import com.jkabe.app.android.banner.view.BannerViewPager;
...
public class Banner1 extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public Banner1(Context context) {
        this(context, null);
    }

    public Banner1(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Banner1(Context context, AttributeSet attrs, int defStyle) {
    ...
    public Banner1 isAutoPlay(boolean isAutoPlay) {
    ...
    public Banner1 setBannerAnimation(Class<? extends ViewPager.PageTransformer> transformer) {
        try {
     ...
     * @return Banner
     */
    public Banner1 setOffscreenPageLimit(int limit) {
        if (viewPager != null) {
    ...
    /**
     * Set a {@link } that will be called for each attached page whenever
     ...
     * @return Banner
     */
    public Banner1 setPageTransformer(boolean reverseDrawingOrder, ViewPager.PageTransformer transformer) {
        viewPager.setPageTransformer(reverseDrawingOrder, transformer);
        return this;
    }

    public Banner1 setBannerTitles(List<String> titles) {
        this.titles = titles;
        return this;
    }
    ...
    public Banner1 setViewPagerIsScroll(boolean isScroll) {
        this.isScroll = isScroll;
        return this;
    }

    public Banner1 setImages(List<ImageInfo> imageUrls) {
        this.imageUrls.addAll(imageUrls);
        this.count = imageUrls.size();
        return this;
    }

    public void update(List<ImageInfo> imageUrls, List<String> titles) {
        this.titles.clear();
        this.titles.addAll(titles);
        update(imageUrls);
    }

    public void update(List<ImageInfo> imageUrls) {
    ...
    public void setTitleView(boolean isShow) {
        if (isShow) {
            titleView.setVisibility(View.GONE);
        } else {
    ...
    private void setImageList(List<ImageInfo> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            Object url = null;
            if (i == 0) {
                url = imagesUrl.get(count - 1).getPhotoFile();
            } else if (i == count + 1) {
                url = imagesUrl.get(0).getPhotoFile();
            } else {
                url = imagesUrl.get(i - 1).getPhotoFile();
            }
            View imageView = null;
            if (imageLoader != null) {
...
//        Log.i(tag,"currentItem: "+currentItem);
    ...
    @Deprecated
    public Banner1 setOnBannerClickListener(OnBannerClickListener listener) {
     ...
     * @return
     */
    public Banner1 setOnBannerListener(OnBannerListener listener) {
        this.listener = listener;
        return this;
    }

    public Banner1 setStartIndex(int index) {
