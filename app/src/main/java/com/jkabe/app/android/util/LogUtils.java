...
public final class LogUtils {
    ...
    public static boolean sLog2ConsoleSwitch = true; // logcat是否打印，默认打印
    public static String sGlobalTag = "日志"; // log标签
    public static boolean sTagIsSpace = false; // log标签是否为空白
    public static boolean sLogHeadSwitch = false; // log头部开关，默认开
    public static boolean sLog2FileSwitch = false;// log写入文件开关，默认关
    public static boolean sLogBorderSwitch = false; // log边框开关，默认开
    public static int sConsoleFilter = V;    // log控制台过滤器
    public static int sFileFilter = V;    // log文件过滤器
    ...
    public static class Builder {
        public Builder() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && BaseApplication.getContext().getExternalCacheDir() != null)
                defaultDir = BaseApplication.getContext().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                defaultDir = BaseApplication.getContext().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
