...
import com.jkabe.app.android.adapter.ReceivedAdapter1;
 ...
 */
public class ReceivedFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener, NetWorkListener {
    ...
    private ReceivedAdapter1 adapter;
    ...
    private void qury() {
        String sign = "memberId=" + SaveUtils.getSaveInfo().getId() + "&partnerid=" + Constants.PARTNERID + "&status=1" + Constants.SECREKEY;
    ...
    public void quryDeil(String orderId) {
        String sign = "id=" + orderId + "&partnerid=" + Constants.PARTNERID + "&status=0" + Constants.SECREKEY;
        showProgressDialog(getActivity(), false);
        Map<String, String> params = okHttpModel.getParams();
        params.put("apptype", Constants.TYPE);
        ...
        params.put("partnerid", Constants.PARTNERID);
        params.put("sign", Md5Util.encode(sign));
        okHttpModel.get(Api.GET_ADVANCE_ORDER, params, Api.GET_ADVANCE_ORDER_ID, this);
    }
    ...
    private void setAdapter(List<OrderInfo> voList) {
        ...
        if (!isRefresh) {
            list.clear();
            list.addAll(voList);
            adapter = new ReceivedAdapter1(this, list);
