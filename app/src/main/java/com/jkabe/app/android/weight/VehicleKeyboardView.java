        ...
        if (keys != null && keys.size() > 0) {
            ...
            for (Keyboard.Key key : keys) {
                ...
                int _3dpToPx = (int) TypedValue.applyDimension(
                        TypedValue.COMPLEX_UNIT_DIP, 2, metrics);
                ...
                // 2 绘制文字
                if (key.label != null) {
                    if (Arrays.asList("ABC".hashCode(), "中文".hashCode(), "完成".hashCode()).contains(key.codes[0])) {
                        paint.setTextSize(bounds.height() - 7 * _6dpToPx);
                    } else {
                        paint.setTextSize(bounds.height() - 7 * _6dpToPx);
                    }
                    if (key.codes[0] == Keyboard.KEYCODE_DONE) {
                        paint.setColor(getContext().getResources().getColor(R.color.black33));
                    } else {
                        paint.setColor(getContext().getResources().getColor(R.color.black33));
                    }
                    @SuppressLint("DrawAllocation")
